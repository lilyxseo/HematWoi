      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        ...
        <div className="rounded-2xl border border-border-subtle bg-surface-alt/60 p-4 sm:p-5">
          ...
          <div className="mt-2 flex items-baseline justify-between gap-3">
            <span className="text-xl font-semibold text-danger">-{formatCurrency(data!.yesterdayExpense)}</span>
            <span className="text-xs font-medium text-muted">{data!.yesterdayCount} transaksi</span>
          </div>
          <p className="mt-1 text-xs text-muted">{data!.yesterdayLabel}</p>
          {data!.yesterdayDetails.length ? (
            <ul className="mt-3 space-y-2 text-sm text-text">
              {data!.yesterdayDetails.map((item) => (
                <li key={item.name} className="flex items-center justify-between">
                  <span className="font-medium">{item.name}</span>
                  <span className="font-semibold text-danger">-{formatCurrency(item.amount)}</span>
                </li>
