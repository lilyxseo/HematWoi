...
export default function Logo({ className, title = DEFAULT_TITLE, ...props }) {
  ...
  const accentGradientId = `${rawId}-accent-gradient`;
  const softGradientId = `${rawId}-soft-gradient`;
  const highlightGradientId = `${rawId}-highlight-gradient`;
    ...
    >
      ...
      <defs>
        <linearGradient id={accentGradientId} x1="0%" y1="100%" x2="100%" y2="0%">
          ...
          <stop offset="100%" stopColor="var(--brand-ring, #312e81)" />
        </linearGradient>
        <linearGradient id={softGradientId} x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" stopColor="var(--brand-soft, #e0e7ff)" stopOpacity="0.9" />
          <stop offset="100%" stopColor="var(--brand, #4f46e5)" stopOpacity="0.2" />
        </linearGradient>
        <linearGradient id={highlightGradientId} x1="0%" y1="0%" x2="100%" y2="0%">
          <stop offset="0%" stopColor="var(--brand, #4f46e5)" stopOpacity="0.9" />
          <stop offset="100%" stopColor="var(--brand-soft, #c7d2fe)" stopOpacity="0.9" />
        </linearGradient>
      </defs>

      <rect width="512" height="512" rx="120" fill={`url(#${softGradientId})`} />
      <rect
        x="36"
        y="36"
        width="440"
