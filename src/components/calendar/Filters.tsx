...
export default function Filters({
  ...
  const hasAdvancedFilters =
    value.type !== 'expense-income' ||
    value.categoryIds.length > 0 ||
    value.accountId !== null ||
    value.minAmount !== null ||
    value.maxAmount !== null;

  const canReset = hasAdvancedFilters || (value.search?.trim()?.length ?? 0) > 0;
  ...
  const handleMinAmountChange = (raw: string) => {
    ...
    onChange({
      ...value,
      minAmount: typeof parsed === 'number' && Number.isFinite(parsed) ? parsed : null,
    });
  };
  ...
  const handleMaxAmountChange = (raw: string) => {
    ...
    onChange({
      ...value,
      maxAmount: typeof parsed === 'number' && Number.isFinite(parsed) ? parsed : null,
    });
  };
            ...
            return (
              <button
                ...
                className={
                  ...
                  (isActive ? 'bg-[var(--accent)]/20 text-slate-100' : 'text-slate-300 hover:bg-slate-800')
                }
      ...
      <div className="flex min-w-0 flex-col gap-2">
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          <label className="text-xs text-slate-400">
            <span className="mb-1 block font-semibold uppercase tracking-wide">Minimal</span>
            <input
              type="number"
              inputMode="numeric"
              pattern="[0-9]*"
          ...
          <label className="text-xs text-slate-400">
            <span className="mb-1 block font-semibold uppercase tracking-wide">Maksimal</span>
            <input
              type="number"
              inputMode="numeric"
              pattern="[0-9]*"
    ...
    <section aria-label="Filter kalender" className="rounded-3xl border border-slate-800 bg-slate-950/70 p-4 shadow-sm">
      <div className="flex flex-col gap-4">
        <div className="flex flex-wrap items-center gap-3">
          <div className="min-w-0 flex-1">
            ...
            <input
              ...
              className="h-10 w-full rounded-2xl border border-slate-700 bg-slate-900 px-3 text-sm text-slate-200 placeholder:text-slate-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]"
            />
          </div>
          <button
            type="button"
            onClick={() => setSheetOpen(true)}
            className={`inline-flex h-10 items-center justify-center rounded-2xl border px-3 text-sm font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] md:hidden ${
              hasAdvancedFilters
          ...
          <button
            ...
            onClick={onReset}
            disabled={!canReset}
            className="hidden h-10 items-center justify-center gap-2 rounded-2xl border border-slate-700 bg-slate-900 px-4 text-sm font-semibold text-slate-200 transition hover:bg-slate-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] disabled:cursor-not-allowed disabled:opacity-60 md:inline-flex"
        ...
        <div className="hidden flex-col gap-4 md:flex">
          <div className="text-sm font-semibold text-slate-200">Filter lanjutan</div>
          {advancedFilterForm}
        </div>
      </div>

      <Transition show={sheetOpen} as={Fragment}>
        <Dialog as="div" className="relative z-50 md:hidden" onClose={setSheetOpen}>
          <Transition.Child
            as={Fragment}
          ...
          <div className="fixed inset-0 z-50 flex items-end justify-center px-3 pb-6">
            <Transition.Child
              as={Fragment}
              enter="ease-out duration-200"
            ...
            >
              <Dialog.Panel className="flex w-full max-w-md flex-col gap-4 rounded-t-3xl border border-slate-800 bg-slate-950 p-4 shadow-xl">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="inline-flex h-9 items-center justify-center gap-2 rounded-2xl border border-slate-700 bg-slate-900 px-3 text-xs font-semibold text-slate-200 transition hover:bg-slate-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]"
                    >
                      <RotateCcw className="h-4 w-4" /> Reset
                    </button>
                    <button
                      ...
                      className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-slate-700 bg-slate-900 text-slate-200 transition hover:bg-slate-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]"
