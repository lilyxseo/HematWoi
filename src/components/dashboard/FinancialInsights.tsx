      ...
      {isLoading ? (
        ...
        <CardBody className="grid gap-3">
          ...
          {overLimitCount > 0 ? (
            <FinancialInsightItem
              icon={<Flame className="h-5 w-5 text-rose-500" aria-hidden="true" />}
              title={`${overLimitCount} kategori melebihi limit`}
              subtitle="Tinjau kategori ini untuk mengendalikan pengeluaran."
              tone="danger"
              badge={{ label: `${overLimitCount}`, tone: 'danger', tooltip: 'Jumlah kategori yang melebihi limit' }}
