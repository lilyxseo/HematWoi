...
function PeriodPicker({ value, preset, onChange, className }: PeriodPickerProps) {
  ...
  return (
    <div className={className}>
      <div className="flex flex-wrap items-center gap-2 rounded-xl border border-border/80 bg-card/60 p-1.5 text-xs shadow-sm backdrop-blur sm:rounded-2xl sm:p-2 sm:text-sm max-[400px]:gap-1.5">
        {SEGMENTS.map((segment) => {
          ...
          return (
            <button
              ...
              className={`inline-flex min-h-[40px] flex-1 items-center justify-center rounded-lg px-3 py-1.5 font-medium transition sm:flex-none sm:rounded-2xl sm:px-4 sm:py-2 ${
                isActive
                  ? "bg-primary text-primary-foreground shadow"
                  : "text-muted-foreground hover:bg-primary/5"
              }`}
              aria-pressed={isActive}
              aria-label={`Pilih periode ${segment.label}`}
            >
              {segment.label}
            </button>
          )
      ...
      {preset === "custom" ? (
        <div className="mt-3 flex flex-wrap items-center gap-3 text-xs sm:text-sm">
          <label className="flex min-w-[140px] flex-1 flex-col gap-1 sm:min-w-[180px]">
            <span className="text-[11px] font-medium text-muted-foreground sm:text-xs">Start</span>
            <input
              type="date"
              value={normalized.start}
              onChange={handleDateChange("start")}
              className="h-10 rounded-xl border border-transparent bg-muted/40 px-3 text-sm shadow-inner ring-2 ring-transparent transition focus:border-primary/40 focus:bg-background focus:outline-none focus:ring-primary/20 sm:h-11 sm:px-4 sm:text-base"
            />
          </label>
          <label className="flex min-w-[140px] flex-1 flex-col gap-1 sm:min-w-[180px]">
