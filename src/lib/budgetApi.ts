...
async function fetchBudgetsForPeriod(userId: string, period: string): Promise<BudgetRow[]> {
  const { data, error } = await supabase
    .from('budgets')
    .select(
      'id,user_id,category_id,planned_amount:amount_planned,carryover_enabled,notes,period_month,created_at,updated_at,category:categories(id,name,type)'
    )
    .eq('user_id', userId)
...
export async function listWeeklyBudgets(period: string): Promise<WeeklyBudgetsResult> {
  ...
  const budgetsPromise = supabase
    .from('budgets_weekly')
    .select(
      'id,user_id,category_id,planned_amount:amount_planned,carryover_enabled,notes,week_start,created_at,updated_at,category:categories(id,name,type)'
    )
    .eq('user_id', userId)
