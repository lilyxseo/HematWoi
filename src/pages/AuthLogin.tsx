...
export default function AuthLogin() {
    ...
    async (event: FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        if (!isEmail) {
          ...
          emailForSignIn = trimmedIdentifier.trim().toLowerCase();
        }
        ...
        } else if (normalized.includes('password') || normalized.includes('kata sandi')) {
  ...
  const handleGoogleLogin = useCallback(async () => {
    ...
    try {
      ...
      if (redirectUrl) {
        ...
        throw new Error('Tidak dapat membuka jendela login Google.');
      }
  ...
  const handleAppleLogin = useCallback(() => {
    setStatus({ type: 'info', message: 'Login dengan Apple belum tersedia.' });
  }, []);
