...
function formatCurrencyValue(value: number) {
  return formatCurrency(Number.isFinite(value) ? value : 0, 'IDR');
}
...
function getStatus(remaining: number, progress: number) {
  if (remaining < 0) {
    return {
      ...
      message: `Pengeluaran sudah melebihi anggaran sebesar ${formatCurrencyValue(Math.abs(remaining))}.`,
    } as const;
  }
  ...
  if (progress >= 90) {
    return {
      ...
      message: `Sisa dana tinggal ${formatCurrencyValue(remaining)} — waktunya rem pengeluaran.`,
    } as const;
  }
  ...
  return {
    ...
    message: `Masih tersedia ${formatCurrencyValue(remaining)} dari anggaran ini.`,
  } as const;
}
  ...
  return (
    <div className={GRID_CLASS}>
      {rows.map((row) => {
        ...
        const remaining = Number.isFinite(Number(row.remaining)) ? Number(row.remaining) : planned - spent;
        const rawPercentage = planned > 0 ? (spent / planned) * 100 : spent > 0 ? 200 : 0;
        const progress = Math.max(0, Math.min(100, Math.round(rawPercentage)));
        const progressTone = getProgressTone(rawPercentage);
        ...
        const percentageLabel = planned > 0 ? `${Math.min(100, Math.round((spent / planned) * 100))}% dari target` : 'Tidak ada batas';
          ...
          <article key={row.id} className={cardClassName} data-highlighted={isHighlighted || undefined}>
            ...
            <header className="relative z-10 flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1 space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="inline-flex items-center shrink-0 rounded-full bg-muted/30 px-3 py-1 text-[0.68rem] font-semibold uppercase tracking-wide text-muted-foreground shadow-sm">
            ...
            <div className="relative z-10 flex flex-wrap items-start justify-between gap-3 text-xs md:text-sm leading-snug text-muted-foreground">
              <div className="min-w-0 flex-1 space-y-2">
                ...
                <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground/80 whitespace-nowrap">
                  Periode {formatRange(row.week_start, row.week_end)}
                </p>
                {notes ? (
            ...
            <div className="relative z-10 flex flex-col gap-3 lg:flex-row lg:flex-wrap lg:items-end lg:justify-between">
              <div className="grid flex-1 gap-3 [grid-template-columns:repeat(auto-fit,minmax(160px,1fr))]">
                <StatItem label="Dialokasikan" value={formatCurrencyValue(planned)} description="Anggaran minggu ini" />
                <StatItem label="Terpakai" value={formatCurrencyValue(spent)} description={percentageLabel} />
                <StatItem
                  label="Sisa"
                  value={formatCurrencyValue(remaining)}
