...
async function buildBalanceSection(userId: string): Promise<string> {
  let balanceSection = "";
  try {
    const b = await getRealtimeBalanceSummary(userId);
    balanceSection = [
    ...
    console.error("[BALANCE SECTION ERROR]", error);
    balanceSection = ["💰 Saldo Saat Ini", "Gagal mengambil saldo."].join("\n");
  }

  console.log("[BALANCE SECTION BUILT]", {
    success: Boolean(balanceSection),
    source: "rpc",
  });
...
function buildTransactionSuccessMessage(input: {
  ...
  warnings?: string[];
  quickCorrectionSection?: string | string[] | null;
}): string {
  const titleByType = input.type === "income"
    ? "✅ Pemasukan Tercatat"
    : input.type === "transfer"
      ? "✅ Transfer Tercatat"
      : "✅ Pengeluaran Tercatat";
  const lines = [titleByType, ""];

  if (input.date) lines.push(`Tanggal: ${input.date}`);
  if (input.type === "transfer") {
    lines.push(`Nominal: ${formatIDR(input.amount)}`);
    lines.push(`Dari: ${input.accountName ?? "-"}`);
    lines.push(`Ke: ${input.toAccountName ?? "-"}`);
  } else {
    lines.push(`Kategori: ${input.categoryName ?? "-"}`);
    lines.push(`Judul: ${input.title ?? "-"}`);
    lines.push(`Nominal: ${formatIDR(input.amount)}`);
    lines.push(`Akun: ${input.accountName ?? "-"}`);
  }

  lines.push("", input.balanceSection || ["💰 Saldo Saat Ini", "Gagal mengambil saldo."].join("\n"));

  if (input.budgetSection) lines.push("", input.budgetSection);
  if (input.warnings && input.warnings.length > 0) lines.push("", ...input.warnings);
  if (input.quickCorrectionSection) {
    if (Array.isArray(input.quickCorrectionSection)) lines.push(...input.quickCorrectionSection);
    else lines.push("", input.quickCorrectionSection);
  }

  const message = lines.join("\n");
  console.log("[TRANSACTION SUCCESS MESSAGE]", {
    hasBalance: message.includes("Saldo Saat Ini"),
    hasBudget: Boolean(input.budgetSection),
  });
  return message;
}
...
function parseSmartTransactionMessage(message: string): ParsedSmartTransaction | ParsedTransactionError | null {
  ...
  return { title, originalTitle, amount, accountName, date: txDate };
}
...
function removeCategoryPrefixFromTitle(rawText: string, categoryName: string): string {
  const fallbackTitle = normalizeTitleText(rawText);
  const normalizedCategory = normalizeTitleText(categoryName);
  if (!fallbackTitle || !normalizedCategory) return fallbackTitle;

  const titleWords = fallbackTitle.split(/\s+/).filter(Boolean);
  const categoryWords = normalizedCategory.split(/\s+/).filter(Boolean);
  if (categoryWords.length === 0 || titleWords.length < categoryWords.length) return fallbackTitle;

  const hasCategoryPrefix = categoryWords.every((word, index) => titleWords[index] === word);
  if (!hasCategoryPrefix) return fallbackTitle;

  return titleWords.slice(categoryWords.length).join(" ").trim();
}

function cleanTransactionTitleWithCategoryPrefix(
  originalTitle: string,
  accountNames: string[],
  categoryName: string,
): string {
  const fallbackTitle = cleanTransactionTitle(originalTitle, accountNames);
  const titleWithoutCategory = removeCategoryPrefixFromTitle(originalTitle, categoryName);
  const sourceTitle = titleWithoutCategory || fallbackTitle;
  ...
  const tokens = title.split(/\s+/).filter(Boolean);
  const keptTokens = tokens.filter((token) => !isAmountToken(token) && !isDateToken(token));
  const cleanedTitle = keptTokens.join(" ")
    .replace(/\b\d{1,2}\/\d{1,2}\b/g, " ")
    .replace(/\s+/g, " ")
    .trim() || fallbackTitle;

  console.log("[CATEGORY PREFIX REMOVED]", {
    categoryName,
    originalTitle,
    cleanedTitle,
  });
...
function parseNaturalTransactionMessage(
  ...
  return { type, accountName: account?.name ?? null, amount, title, originalTitle, date };
}
...
function parseDebtPaymentCommand(message: string): ParsedDebtPaymentCommand | null {
  const parts = message.trim().split(/\s+/).filter(Boolean);
  if (parts.length < 4) return null;
  const action = normalizeText(parts[0]);
  const debtWord = normalizeText(parts[1]);
  if (action !== "bayar" || (debtWord !== "hutang" && debtWord !== "piutang")) return null;
  ...
  const core = workParts.slice(2);
  const amountIndex = core.findIndex((part) => parseAmount(part) > 0);
  if (amountIndex <= 0) return null;

  const selector = core.slice(0, amountIndex).join(" ").trim();
  const amount = parseAmount(core[amountIndex]);
  const accountName = core.slice(amountIndex + 1).join(" ").trim() || null;
  if (!selector || amount <= 0) return null;

  const selectorNumber = /^\d+$/.test(selector) ? Number(selector) : null;
  return {
    debtType,
    selector,
    selectorNumber,
    ...
    accountName,
    date,
    paymentSource: selectorNumber !== null ? "number" : "name",
  };
}
...
async function completePendingTransaction(userId: string, session: PendingTransactionSession, sourceMessage: string): Promise<{ reply: string; parsedLog: Record<string, JsonValue> }> {
  ...
  const [balanceSection, budgetSection] = await Promise.all([
    buildBalanceSection(userId),
    buildTransactionBudgetSection({ userId, categoryId: category.id, amount: session.amount, transactionDate: session.date }),
  ]);
  const reply = buildTransactionSuccessMessage({
    type,
    categoryName: category.name,
    title: session.title,
    amount: session.amount,
...
async function getLastTransactionSession(userId: string, phone: string): Promise<LastTransactionSession | null> {
  ...
  const sessionLog = (logs ?? []).find((log) => {
    const parsed = (log.parsed ?? null) as Record<string, JsonValue> | null;
...
function parseQuickFixCommand(normalized: string): { field: "category" | "account"; value: string } | null {
  const match = normalized.match(/^(kategori|akun)\s+(.+)$/);
  if (!match) return null;
  const field = match[1] === "kategori" ? "category" : "account";
  const value = String(match[2] ?? "").trim();
  if (!value) return null;
  if (["tambah", "edit", "hapus", "list"].includes(value.split(" ")[0] ?? "")) return null;
  return { field, value };
}
...
async function handleQuickFixCommand(
  ...
  if (!session) {
    return {
      reply: ["⚠️ Tidak ada transaksi terakhir yang bisa diedit.", "", "Gunakan:", "history"].join("\n"),
      parsedLog: { command: "quick_fix_failed", field: command.field, reason: "last_transaction_not_found" },
    };
  }
  ...
  if (!txRow) {
    return {
      reply: ["⚠️ Tidak ada transaksi terakhir yang bisa diedit.", "", "Gunakan:", "history"].join("\n"),
      parsedLog: { command: "quick_fix_failed", field: command.field, reason: "transaction_not_found", transactionId: session.transactionId },
    };
  }
  ...
  if (command.field === "category") {
    ...
    const oldCategoryId = String(txRow.category_id ?? session.categoryId ?? "");
    const { data: oldCategoryRow } = oldCategoryId
      ...
      : { data: null };
    const oldCategory = String(oldCategoryRow?.name ?? session.categoryName ?? "-");
    await updateTransactionRequiredFields(session.transactionId, userId, { category_id: newCategory.id, type: newCategory.type });
    await updateTransactionOptionalField(session.transactionId, userId, "category_name", newCategory.name);
    await updateTransactionOptionalField(session.transactionId, userId, "category", newCategory.name);
    console.log("[QUICK CATEGORY FIX]", {
      transactionId: session.transactionId,
      oldCategory,
  ...
  const oldAccountId = String(txRow.account_id ?? session.accountId ?? "");
  const { data: oldAccountRow } = oldAccountId
    ? await supabase.from("accounts").select("name").eq("id", oldAccountId).eq("user_id", userId).maybeSingle()
    : { data: null };
  const oldAccount = String(oldAccountRow?.name ?? session.accountName ?? "-");
  await updateTransactionRequiredFields(session.transactionId, userId, { account_id: newAccount.id });
  await updateTransactionOptionalField(session.transactionId, userId, "account_name", newAccount.name);
  await updateTransactionOptionalField(session.transactionId, userId, "account", newAccount.name);
  console.log("[QUICK ACCOUNT FIX]", {
    transactionId: session.transactionId,
    oldAccount,
    newAccount: newAccount.name,
  });

  return {
    reply: [
      "✏️ *Akun Berhasil Diubah*",
      "",
      "━━━━━━━━━━━━━━",
      "Sebelum:",
      `*${oldAccount}*`,
...
async function findOpenDebtsByParty(userId: string, debtType: DebtType, partyName: string): Promise<Array<Record<string, JsonValue>>> {
  const { data, error } = await supabase
    .from("debts")
    .select("*")
    .eq("user_id", userId);
...
async function findOpenDebtById(userId: string, debtId: string, debtType: DebtType): Promise<Record<string, JsonValue> | null> {
  const { data, error } = await supabase
    .from("debts")
    .select("*")
    .eq("id", debtId)
  ...
  if (!isDebtRowOpen(row)) return null;
  const hasTypeField = rowHasDebtTypeField(row);
  if (hasTypeField && !isDebtTypeRow(row, debtType === "debt" ? "hutang" : "piutang")) return null;
  if (!hasTypeField && debtType !== "debt") return null;
  return row;
}


function accountHasPrimaryFlag(row: Record<string, JsonValue>): boolean {
  return row.is_default === true ||
    ...
    row.main === true ||
    row.is_utama === true ||
    normalizeText(String(row.name ?? "")) === "utama";
}

async function findPrimaryAccount(userId: string): Promise<{ id: string; name: string; type: string } | null> {
  const { data, error } = await getAccountsBaseQuery(userId, "*");
  if (error) throw error;
  const rows = (data ?? []) as Array<Record<string, JsonValue>>;
  const row = rows.find(accountHasPrimaryFlag) ?? (rows.length === 1 ? rows[0] : null);
  if (!row) return null;
  return { id: String(row.id), name: String(row.name), type: String(row.type ?? "") };
}

async function findLastDebtPaymentAccount(userId: string, debtType: DebtType): Promise<{ id: string; name: string; type: string } | null> {
  const rows = await getOpenDebts(userId);
  const debtIds = rows
    .filter((row) => {
      const hasTypeField = rowHasDebtTypeField(row);
      if (hasTypeField) return isDebtTypeRow(row, debtType === "debt" ? "hutang" : "piutang");
  ...
  const { data, error } = await supabase
    ...
    .limit(1);
  if (error) throw error;
  const accountId = String(((data ?? []) as Array<Record<string, JsonValue>>)[0]?.account_id ?? "");
  if (!accountId) return null;
  return await getAccountById(userId, accountId);
}

async function resolveDebtPaymentAccount(userId: string, debtType: DebtType, accountName: string | null): Promise<{ account: { id: string; name: string; type: string } | null; status: string; requestedName: string | null }> {
  if (accountName) {
    const account = await findAccount(userId, accountName);
    return { account, status: account ? "explicit" : "not_found", requestedName: accountName };
  }

  const lastPaymentAccount = await findLastDebtPaymentAccount(userId, debtType);
  if (lastPaymentAccount) return { account: lastPaymentAccount, status: "last_payment", requestedName: null };

  const primaryAccount = await findPrimaryAccount(userId);
  if (primaryAccount) return { account: primaryAccount, status: "primary", requestedName: null };

  return { account: null, status: "needs_account", requestedName: null };
}

function getDebtPaidField(row: Record<string, JsonValue>): string | null {
  return "paid_amount" in row
    ? "paid_amount"
    : "paid" in row
    ? "paid"
    : "amount_paid" in row
...
function getDebtDisplayName(row: Record<string, JsonValue>, fallback: string): string {
  const name = getDebtName(row);
...
async function processDebtPayment(input: {
  ...
  const { userId, rawMessage, debtType, debt, amount, account, date, paymentSource, selectorName } = input;
  ...
  const debtName = getDebtDisplayName(debt, selectorName);

  if (amount > remaining) {
    return {
      reply: `⚠️ Nominal melebihi sisa hutang.\n\nSisa hutang: ${formatIDR(remaining)}`,
      parsedLog: { command: "debt_payment", status: "amount_exceeds_remaining", debtId, amount, accountId: account.id, paymentSource },
    };
  }
  ...
  if (!paidField) {
    return { reply: "❌ Kolom pembayaran hutang tidak ditemukan di schema.", parsedLog: { command: "debt_payment", status: "payment_field_not_found", debtId, amount, accountId: account.id, paymentSource } };
  }
  ...
  const { error: updateDebtErr } = await supabase.from("debts").update(debtUpdate).eq("id", debtId).eq("user_id", userId);
  ...
  if (!paymentCategory?.name) warnings.push("⚠️ Kategori Hutang/Piutang atau Lainnya belum ditemukan, transaksi dicatat tanpa kategori.");
...
async function findLastDisplayedDebtList(userId: string, debtType?: DebtType): Promise<Array<Record<string, JsonValue>> | null> {
  const { data, error } = await supabase
    ...
    .limit(50);
  if (error) throw error;
  const expectedLegacyCommand = debtType === "receivable" ? "piutang_list" : "hutang_list";
  const expectedType = debtType === "receivable" ? "piutang" : debtType === "debt" ? "hutang" : null;
  for (const row of (data ?? []) as Array<Record<string, JsonValue>>) {
    const parsed = row.parsed as Record<string, JsonValue> | null;
    if (!parsed) continue;
    const command = String(parsed.command ?? "");
    const parsedType = String(parsed.debtType ?? parsed.type ?? "");
    const matchesCommand = command === "debt_list"
      ? !expectedType || parsedType === expectedType || parsedType === debtType
      : command === expectedLegacyCommand || (!debtType && (command === "hutang_list" || command === "piutang_list"));
  ...
  if (parsedPayment) {
    ...
    if (parsedPayment.paymentSource === "number") {
      ...
      if (matches.length > 1) {
        ...
        console.log("[DISPLAYED DEBTS]", displayedDebts);
        const typeWord = parsedPayment.debtType === "debt" ? "hutang" : "piutang";
        const suggestions = (displayedDebts ?? [])
          .filter((item) => matches.some((match) => String(match.id ?? "") === String((item as Record<string, JsonValue>).id ?? "")))
          .map((item) => `• bayar ${typeWord} ${Number((item as Record<string, JsonValue>).no ?? 0)} ${parsedPayment.amount}`)
      ...
      if (transfer) {
        ...
        if (tx && "error" in tx && tx.error === "INVALID_DATE") {
          ...
          if (!category) {
            ...
            if (smartTx && !("error" in smartTx)) {
              ...
              if (smartTx.accountName && !smartAccount) {
                ...
                if (!smartCategory) {
                  ...
                  const [balanceSection, budgetSection] = await Promise.all([
                    buildBalanceSection(userId),
                    buildTransactionBudgetSection({ userId, categoryId: smartCategory.id, amount: smartTx.amount, transactionDate: smartTx.date }),
                  ]);
                  const warnings = smartType === "expense" ? await buildSmartWarnings({ userId, date: smartTx.date, amount: smartTx.amount, categoryName: smartCategory.name }) : [];
                  if (smartType === "expense") console.log("[SMART WARNING]", { warnings, transactionAmount: smartTx.amount, categoryName: smartCategory.name });
                  reply = buildTransactionSuccessMessage({
                    type: smartType,
                    categoryName: smartCategory.name,
                    title: finalSmartTitle,
                    amount: smartTx.amount,
                    accountName: smartAccount.name,
                    date: smartTx.date,
                    balanceSection,
                    budgetSection: smartType === "expense" ? budgetSection : null,
                    warnings,
                    quickCorrectionSection: buildQuickFixLines(smartAccount.name),
                  });
                }
              }
            } else {
            ...
            const [balanceSection, budgetSection] = await Promise.all([
              buildBalanceSection(userId),
              buildTransactionBudgetSection({ userId, categoryId: category.id, amount: tx.amount, transactionDate: tx.date }),
            ]);
            ...
            reply = buildTransactionSuccessMessage({
              ...
              date: tx.date,
              balanceSection,
              budgetSection: type === "expense" ? budgetSection : null,
              warnings,
              quickCorrectionSection: buildQuickFixLines(account.name),
            });
          }
        } else {
          const smartTx = parseSmartTransactionMessage(normalized);
          if (!smartTx) {
            ...
            if (!naturalTx) {
              ...
              if (!category) {
                ...
                if (naturalTx.accountName && !account) {
                  ...
                  const [balanceSection, budgetSection] = await Promise.all([
                    buildBalanceSection(userId),
                    buildTransactionBudgetSection({ userId, categoryId: category.id, amount: naturalTx.amount, transactionDate: naturalTx.date }),
                  ]);
                  const warnings = finalType === "expense" ? await buildSmartWarnings({ userId, date: naturalTx.date, amount: naturalTx.amount, categoryName: category.name }) : [];
                  if (finalType === "expense") console.log("[SMART WARNING]", { warnings, transactionAmount: naturalTx.amount, categoryName: category.name });
                  reply = buildTransactionSuccessMessage({
                    type: finalType,
                    categoryName: category.name,
                    title: finalNaturalTitle,
                    ...
                    date: naturalTx.date,
                    balanceSection,
                    budgetSection: finalType === "expense" ? budgetSection : null,
                    warnings,
                    quickCorrectionSection: buildQuickFixLines(account.name),
                  });
                }
            ...
            if (smartTx.accountName && !account) {
              ...
              if (!category) {
                ...
                const [balanceSection, budgetSection] = await Promise.all([
                  buildBalanceSection(userId),
                  buildTransactionBudgetSection({ userId, categoryId: category.id, amount: smartTx.amount, transactionDate: smartTx.date }),
                ]);
                const warnings = type === "expense" ? await buildSmartWarnings({ userId, date: smartTx.date, amount: smartTx.amount, categoryName: category.name }) : [];
                if (type === "expense") console.log("[SMART WARNING]", { warnings, transactionAmount: smartTx.amount, categoryName: category.name });
                reply = buildTransactionSuccessMessage({
                  type,
                  categoryName: category.name,
                  title: finalSmartTitle,
