...
begin
  if not exists (
    ...
    where schemaname = 'auth'
      ...
      and policyname = 'user_sessions_select'
  ) then
    execute $$create policy user_sessions_select on auth.sessions
      for select
  ...
  if not exists (
    ...
    where schemaname = 'auth'
      and tablename = 'sessions'
      and policyname = 'user_sessions_delete'
  ) then
    execute $$create policy user_sessions_delete on auth.sessions
      for delete
      using (user_id = auth.uid())$$;
  end if;
end;
